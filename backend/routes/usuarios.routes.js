...
// GET /api/usuarios/pacientes — Médicos pueden buscar pacientes para prescribir
router.get('/pacientes', requireAuth, requireRole('admin','superadmin','medico','farmacia'), async (req, res) => {
  try {
    const { q } = req.query;
    let sql = `SELECT id, nombre, apellido, email FROM usuarios WHERE rol='paciente' AND activo=true`;
    const params = [];
    if (q) {
      params.push('%' + q.toLowerCase() + '%');
      sql += ` AND (LOWER(nombre||' '||apellido) LIKE $1 OR LOWER(email) LIKE $1)`;
    }
