...
--
--  SUPABASE: Ejecutar en SQL Editor de tu proyecto Supabase
--  LOCAL:    psql -U postgres -d smarthealth -f schema.sql
--
--  NOTA SUPABASE: gen_random_uuid() ya está disponible.
--  pgcrypto puede necesitar ejecutarse como superadmin.
-- ═══════════════════════════════════════════════════════

-- En Supabase esto ya existe; en local es necesario
...
CREATE TABLE IF NOT EXISTS usuarios (
  ...
  habeas_consent BOOLEAN     NOT NULL DEFAULT TRUE,
  habeas_fecha  TIMESTAMP    NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS formulas (
  ...
  farmacia            VARCHAR(80),
  estado              VARCHAR(20) NOT NULL DEFAULT 'pendiente' CHECK (estado IN ('pendiente','preparando','en_camino','entregado','cancelado')),
